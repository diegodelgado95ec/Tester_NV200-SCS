...
_MODULUS   = 4611686018427387847  # primo de 62 bits
# ────────────────────────────────────────────────────────
...
class eSSPCrypto:
    ...
    def negotiate(self, drv) -> bool:
        ...
        try:
            ...
            code, extra, _ = drv.send(0x4C, _int_to_8le(host_inter))
            print(f"  REQUEST KEY   → 0x{code:02X}  extra={extra.hex() if extra else 'vacío'}")
           ...
           # DEBUG — agrega estas líneas:
            ...
            self._aes_key      = _int_to_8le(self._fixed_key) + _int_to_8le(neg_key)
            self._count_enc    = 0
            self._count_dec    = 1
